...
//        trackView_.frame = CGRectMake(trackX, trackView_.frame.origin.y, CGRectGetWidth(trackView_.bounds), CGRectGetHeight(trackView_.bounds));
        ...
        [self viewTobuttonIndex:_selectedIndex];
...
- (void)viewTobuttonIndex:(NSInteger)index {
    
    //[self.chooseViewDelegate chooseViewIndex:index];
}
...
- (void)selectAt:(NSInteger)index {
    ...
    if (self.delegate) {
        [self.delegate DLSlideTabbar:self selectAt:index];
    }
